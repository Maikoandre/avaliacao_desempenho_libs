        ...
        if not IS_POWERFUL_PC:
            ...
            if op == "sort": 
                ...
                if os.path.getsize(file_path) / (1024**3) > 0.4:
                    ...
                    sys.exit(0)
                track_metrics_internal(lib, size, op, lambda: q.sort("DT_NOTIFIC").collect(engine='streaming'))
        else:
            # Eager mode for powerful PC
            df = pl.read_parquet(file_path)
            if op == "filter": track_metrics_internal(lib, size, op, lambda: df.filter(pl.col("ID_MUNICIP") == 355030))
            if op == "aggr": track_metrics_internal(lib, size, op, lambda: df.group_by("CS_SEXO").agg(pl.col("NU_IDADE_N").sum()))
            if op == "join": track_metrics_internal(lib, size, op, lambda: df.join(lk_pl, on="CS_SEXO"))
            if op == "sort": track_metrics_internal(lib, size, op, lambda: df.sort("DT_NOTIFIC"))
